...
Semaphore::Semaphore(const char* nombre, int valorInicial, char letra) {
...
int Semaphore::p(int val) {
	struct sembuf operacion;

	operacion.sem_num = 0; // numero de semaforo
	operacion.sem_op = val; // restar 1 al semaforo
...
int Semaphore::v(int val) {

	struct sembuf operacion;

	operacion.sem_num = 0; // numero de semaforo
	operacion.sem_op = val; // sumar 1 al semaforo
	operacion.sem_flg = SEM_UNDO;
